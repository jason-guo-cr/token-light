...
static void drawQuotaCard(U8G2 &u8g2, const LimitWindow &window) {
  const int x = 20;
  const int y = 140;
  u8g2.drawRFrame(x, y, 360, 98, 6);
  u8g2.setFont(u8g2_font_9x18B_tf);
  u8g2.drawStr(x + 16, y + 25, window.label.c_str());

  u8g2.setFont(u8g2_font_6x13B_tf);
  String reset = "RESET " + window.resetLabel;
  int resetWidth = u8g2.getStrWidth(reset.c_str());
  u8g2.drawStr(x + 344 - resetWidth, y + 22, reset.c_str());

  u8g2.setFont(u8g2_font_logisoso26_tn);
  char percent[8];
  snprintf(percent, sizeof(percent), "%d%%", window.remainingPercent);
  u8g2.drawStr(x + 16, y + 67, percent);

  u8g2.setFont(u8g2_font_9x18B_tf);
  u8g2.drawStr(x + 104, y + 64, "LEFT");
  u8g2.setFont(u8g2_font_6x13B_tf);
  char used[16];
  snprintf(used, sizeof(used), "USED %d%%", window.usedPercent);
  int usedWidth = u8g2.getStrWidth(used);
  u8g2.drawStr(x + 344 - usedWidth, y + 62, used);
  drawProgress(u8g2, x + 16, y + 78, 328, window.remainingPercent);
}
...
void renderDashboard(U8G2 &u8g2, const DisplaySnapshot &snapshot, unsigned long nowMs) {
  ...
  if (snapshot.limitUpdatedLabel.length() > 0) {
    String updated = "SYNC " + snapshot.limitUpdatedLabel;
    u8g2.setFont(u8g2_font_6x13B_tf);
  ...
  if (snapshot.status == "live" || snapshot.status == "cached") {
    drawQuotaCard(u8g2, snapshot.primary);
  } else {
